...
function makeSVGResizer(obj) {
  var idstr = obj.id;
  return function () {
    if (document.getElementById(idstr)==undefined) return;
    // allow flex box to work
    mqAttr(idstr+'-svg','width','1');
    mqAttr(idstr+'-svg','height','1');
    setTimeout(function() {
      var w = mqWidth(idstr);
      var h = mqHeight(idstr);
      mqAttr(idstr+'-svg','width',w);
      mqAttr(idstr+'-svg','height',h);
      mqAttr(idstr+'-svg','viewBox','0 0 ' + w + ' ' + h);
      obj.plot();
    },10);
  }
}
...
class Plotter { 
  ...
  populate_annotations() {
    ...
    for (var i=0;i<signalbase.data.annotations.length;++i) {
      ...
      this.annotations.push( [xpos,label]);
    }
  }
  plot_annotations() {
  ...
  plot () {
    ...
    var that = this;
    setTimeout( function () { that.plot_internal(); }, 100.0);
  }
}
