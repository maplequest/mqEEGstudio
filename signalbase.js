...
/*
    var ann_signal = -1;
    for (var i=0;i<ns;i++) {
      if (this.data.labels[i]=="EDF Annotations") ann_signal=i;
    }
    if (ann_signal!=-1) {
      this.data.signals[ann_signal]=[];
      for (var i=0;i<this.data.annotations.length;i++) {
        var tmp = this.write_annotation(this.data.annotations[i]);
        this.data.signals[ann_signal]=this.data.signals[ann_signal].concat(tmp);
      }
      var ann_srate = Math.ceil(this.data.signals[ann_signal].length/rn);
      nr[ann_signal]=Math.floor(ann_srate*rt);
      for (var i=this.data.signals[ann_signal].length;i<ann_srate*rn;i++) {
        this.data.signals[ann_signal].push(0);
      }
    }
*/
